...
def resolve_clean_targets(targets: List[str], bucket: str = "source") -> List[str]:
    filenames: List[str] = []
    if targets:
...
def run_image_cleaning(
    ...
    append_log("ai_clean", f"[{get_timestamp()}] 🤖 当前模型：{model}")
    ...
    update_state(
        ...
        progress=100,
        processed=processed_count,
        message=f"清洗完成，成功 {success_count} 张，失败 {failed_count} 张",
    )
    return results


def queue_cleaning(payload: dict) -> tuple[bool, str, List[Dict] | None]:
    if not payload["model"] or not payload["api_key"]:
        return False, "请先在 AI 平台配置中填写模型名称与 API Key", None

    filenames = resolve_clean_targets(payload["targets"])
    if not filenames:
        return False, "未找到可清洗的图片，请先上传", None

    with state_lock:
        if task_state["ai_clean"]["status"] == "running":
            return False, "已有 AI 图片清洗任务正在执行", None
        update_state(
            "ai_clean",
            status="queued",
            progress=0,
