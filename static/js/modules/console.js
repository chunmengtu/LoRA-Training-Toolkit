...
export function applySectionState(section, data) {
    if (!data) return;
    const prefixMap = {setup: "setup", download: "download", generation: "generation", ai_clean: "aiClean", ai_tag: "aiTag"};
    const prefix = prefixMap[section];
    if (!prefix) return;

    const pageMap = {setup: "setup", download: "download", generation: "ai", ai_clean: "ai-clean", ai_tag: "ai-tag"};
